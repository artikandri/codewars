...
function slicesToName(n) {
  if (!n || n < 2 || typeof n !== "number") return null;
  const sandwich = Math.floor(n / 2);
  const bread = n - sandwich * 2;
  return (
    (bread ? "bread ".repeat(bread) : "") + "sandwich ".repeat(sandwich)
  ).trim();
}
...
function nameToSlices(name) {
  if (
    !name ||
    typeof name !== "string" ||
    name.includes("sandwich bread sandwich")
  )
    return null;
  const slices = name
    .split(" ")
    .filter((x) => x === "bread" || x === "sandwich");
  const sliceCount = slices.reduce(
    (a, b, i) => a + (b === "sandwich" ? 2 : 1),
    0
  );
  return sliceCount < 2 ? null : sliceCount;
}
